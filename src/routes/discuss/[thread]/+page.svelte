<script lang="ts">
	...
	// On-site thread reader (revived per ruling D15; privacy SSOT:
	// docs/spec/discuss-board-lifecycle-2026-09-01.md). Renders ONE archived
	// discuss@ thread fully on-site so a reader is never dumped into unstyled
	// HyperKitty/Postorius. The server load hands us a sanitized, privacy-gated
	// DiscussThreadDetail — or `null`, which is the calm "unavailable" state,
	// NOT an error page. Each message body arrives as pre-sanitized paragraph
	// blocks carrying a quotation depth; quotation reads via indentation +
	// muted ink (house canon: never a side-stripe border). Hairline dividers
	// separate messages; long subjects and unbroken tokens wrap (375px-safe)
