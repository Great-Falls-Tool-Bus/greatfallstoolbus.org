...
/** The keyholders Mailman surface. Post address is live; list runtime is the
 *  next proof gate. */
export const LIST = {
...
/** HyperKitty web archive. URL is a placeholder until TIN-2380 lands the list
 *  runtime and archive host. */
export const ARCHIVE = {
	url: 'https://lists.latoolb.us/hyperkitty/list/keyholders@latoolb.us/',
	status: 'pending (TIN-2380)',
} as const;
...
export interface MailClient {
	...
	summary: string;
	/** How to subscribe by email to the keyholders list. */
	subscribe: string;
...
export const MAIL_CLIENTS: MailClient[] = [
	{
		...
		subscribe: `Send an empty email to ${LIST.join} from your Gmail address, then confirm the reply.`,
		...
		agentHints: [
			`Compose an email to ${LIST.join} with an empty subject and body, send it, and watch for the confirmation reply.`,
	...
	},
	{
		...
		subscribe: `Send an empty email to ${LIST.join}, then confirm the reply.`,
		...
		agentHints: [
			`Send an empty message to ${LIST.join} and confirm the round trip.`,
	...
	},
	{
		...
		mailbox: `For a latoolb.us mailbox, add an account with ${imapSmtp}`,
		agentHints: [
			`Send an empty message to ${LIST.join} and confirm the reply.`,
	...
	},
	{
		...
		summary: `Lightweight client; sort list mail server-side or via the web archive.`,
		subscribe: `Send an empty email to ${LIST.join}, then confirm the reply.`,
