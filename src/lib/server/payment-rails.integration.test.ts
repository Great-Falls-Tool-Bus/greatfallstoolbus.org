...
import { recordCashCheckReceipt, reverseReceipt } from './contribution/receipt';
...
import { signPayloadForTest } from './stripe/client';
import type { StripeWebhookSecret } from './stripe/config';
import { FIXTURE, createReplayGateway, readFixtureEventRaw } from './stripe/fixtures';
...
describe('cash and check are first-class rails', () => {
	...
	it('is finance data another tenant cannot see at all', async () => {
		const rows = await asTenant(fixture.runtimeDsn, tenantB, async (client) => {
			const { rows: receipts } = await client.query(`select id from finance_receipt`);
			return receipts;
		});
		expect(rows).toEqual([]);
	});
