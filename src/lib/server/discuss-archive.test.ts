...
// Unit suite for the discuss data plane (revived per ruling D15; privacy SSOT:
// docs/spec/discuss-board-lifecycle-2026-09-01.md). This is a PUBLIC repo
// (scripts/scan-internal-endpoints.sh + the `internal-hostname` leak rule in
// scripts/lib/leak-scan-rules.json): no real namespace and no dotted private
// address may be a literal in a tracked file. The in-cluster origins under
// test are built through the module's own `inClusterOrigin()` with a
// throwaway namespace, and the one private IP the Host-header negative case
// needs is assembled from octets — the same technique
...
describe('assertSnapshotIsPublicSafe (privacy gate; spec: discuss-only, no addresses)', () => {
...
describe('fetchDiscussSnapshot', () => {
	...
	it('hydrates concurrently but keeps deterministic thread order', async () => {
		// Three threads whose per-thread reads resolve in scrambled order; the
		// bounded-concurrency hydration must still emit lastActiveAt-desc order.
		const threads = ['T1', 'T2', 'T3'];
		const list = threads.map((id, i) => ({
			thread_id: id,
			subject: `[Discuss] ${id}`,
			date_active: `2026-07-0${i + 1}T00:00:00Z`, // T3 most recent
			replies_count: 0,
			starting_email: `http://svc/hyperkitty/api/list/discuss@latoolb.us/email/E${id}/?format=json`,
		}));
		const delays: Record<string, number> = { T1: 3, T2: 1, T3: 2 };
		const impl = vi.fn(async (url: string | URL) => {
			const key = String(url);
			if (key.includes('/threads/')) return { ok: true, status: 200, json: async () => list } as Response;
			const id = threads.find((t) => key.includes(t));
			if (!id) return { ok: false, status: 404, json: async () => ({}) } as Response;
			await new Promise((resolve) => setTimeout(resolve, delays[id]));
			if (key.includes('/thread/')) {
				const item = list.find((l) => l.thread_id === id);
				return {
					ok: true,
					status: 200,
					json: async () => ({ ...item, participants_count: 1 }),
				} as Response;
			}
			return {
				ok: true,
				status: 200,
				json: async () => ({ sender_name: 'A Neighbor', date: '2026-07-01T00:00:00Z', content: 'hi' }),
			} as Response;
		}) as unknown as typeof fetch;
		const snap = await fetchDiscussSnapshot({ origin: 'http://svc:8080', fetch: impl });
		expect(snap.threads.map((t) => t.threadId)).toEqual(['T3', 'T2', 'T1']);
	});
...
// ============================================================================
// On-site thread reader: sanitizer + mapper + privacy gate + fetch.
// ============================================================================
