<script lang="ts">
	// On-site archive thread index (revived per ruling D15; privacy SSOT:
	// docs/spec/discuss-board-lifecycle-2026-09-01.md). Renders the discuss
	// snapshot as a hairline-divided list of threads (subject → meta line →
	// excerpt, separated by top hairlines), never a card grid. Each subject
	// links INTERNALLY to the on-site reader at `/discuss/<threadId>` (the full
	// conversation renders on-site, never dumping a reader into unstyled
	// HyperKitty); the external deep link lives on that reader page.
	// `DiscussThread.url` intentionally still holds the public HyperKitty URL
...
<div class="mt-6">
	...
	{#if threads.length === 0}
		<!-- Calm empty state: no threads yet is a normal, healthy state, not an
		     error. No "start one" invitation here — posting comes with membership
		     (spec read/write matrix; the page above this component carries the
		     read-is-free / write-requires-membership disclosure). -->
		<p class="text-surface-600-400 border-surface-200-800 mt-4 border-t py-10 leading-relaxed">
			No conversations have been archived yet. Check back soon.
		</p>
	{:else}
		<ul class="mt-4">
