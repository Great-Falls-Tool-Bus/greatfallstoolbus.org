...
- **Declare-only (TIN-2360 row c, DECIDED 2026-07-02).** GFTB IaC lives on
  three planes: declare-only tofu/intent in this public repo; runtime apply
  (mail / list / Anubis / tunnel / DNS) in blahaj per the MassageIthaca
  precedent; runner tenancy in `great-falls-tool-bus-infra` per TIN-2299.
  This directory is the first plane only.
...
- **Zero secrets (row d).** The names of the secrets this intent will
  ...
  at the repo root. Values live blahaj-side under the
  `tenants/great-falls-tool-bus/secrets/` sops+age lane.
- **Nothing here unblocks work by itself.** This is prep. Mail cutover
  ...
  (row g), and the blahaj-side deployments carry their own ADRs before
...
| `keyholders@latoolb.us` | First production `MailAccount` application (`mail.tinyland.dev/v1alpha1`, reconciled blahaj-side) |
| `keyholders` list | Mailman 3 + Postorius + HyperKitty, pinned trio 3.3.10 / 1.3.13 / 1.3.12, public archive (row e) |
| Anubis v1.25.0 | Behind the blahaj Cloudflare tunnel; default-allow with a single CHALLENGE rule scoped to the join/contact form route only; archive, `/agent`, `/llms.txt`, and all SEO surfaces stay unguarded (row f) |
