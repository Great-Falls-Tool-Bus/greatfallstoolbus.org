...
//   - naming-consent (src/lib/naming-consent.ts): no private-only identity,
//     the bus host only as "J."
//   - no bare email address in subject or body (the two list addresses,
//     which are not personal, are the only allowed exceptions)
...
export function frontmatterBlock(fm) {
	const lines = [
		...
		`redactionsApplied: ${fm.redactionsApplied}`,
		`consentVerified: ${fm.consentVerified}`,
	];
	return `---\n${lines.join('\n')}\n---\n`;
}
...
export function buildDraft(raw, preparedAt) {
	...
	// Naming-consent + address gates run on the operator-authored prose only
	// (subject, body) — the list literals below are fixed, schema-enforced
	// constants, not personal data, so they are not scanned here.
